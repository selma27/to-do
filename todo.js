/* add element - press enter */
document.querySelector('#taskInput').addEventListener('keypress', function (e) {
    var key = e.which || e.keyCode;
    if (( key == 13 ) && ( document.querySelector('#taskInput').value.length > 0 ))     	addToList( this.value.trim() );
});
...
/* add element - click addBtn */
function newElement() {
document.querySelector('#addBtn').addEventListener('click',
function(){
     var elem=document.querySelector('#taskInput').value;
    if(elem.length>0)
    addToList(elem);
}
 );  }
...
/*
export default function funExport() {
}
*/